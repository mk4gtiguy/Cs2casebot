...
def apply_house(raw: float) -> float:
    return round(raw * (1 - HOUSE_EDGE), 2)

async def log_game(conn, user_id: int, game_type: str,
                   bet: float, win: float, meta: dict = None):
    import json
    mult = round(win / bet, 4) if bet else 0
    await conn.execute("""
        INSERT INTO game_logs
            (user_id, game_type, bet_amount, win_amount, multiplier, result, meta)
        VALUES ($1,$2,$3,$4,$5,$6,$7)
    """, user_id, game_type, bet, win, mult,
